...
const overrides = new Map([
  ...
  ["src/features/channels/ui/ChannelScreen.tsx", 625], // profile panel state + mutual exclusion wiring + ProfilePanelProvider context + agent typing classification + imetaMedia projection on editTarget + thread follow wiring from AppShell context + WalletBot local-DM guardrails/profile injection + shared-agent payment profile lookups
  ...
  ["src/features/messages/hooks.ts", 620], // message query/mutation hooks + optimistic updates + WalletBot local-message bridge + kudos/shared-agent payment hooks
  ...
  ["src-tauri/src/managed_agents/types.rs", 747], // ManagedAgentRecord/Summary + Create/Update request structs + AcpProviderCatalogEntry + InstallRuntimeResult + RespondTo enum + validate_respond_to_allowlist + tests + persona/agent env_vars field + owner pubkey summary field
  ...
  ["src-tauri/src/lib.rs", 787], // +4 lines for PairingHandle managed state + 3 pairing command registrations + parse_message_deep_link helper extracted with 6 unit tests covering empty-param filter regression + mod migration + sync_shared_agent_data/reconcile_provider_mcp_commands/reconcile_persona_pack_paths calls on launch + SIGINT/SIGTERM/SIGHUP signal handlers for agent process cleanup + WalletBot command registration + wallet profile-payment command registration + shared-agent owner resolver command
  ["src-tauri/src/commands/agent_discovery.rs", 510], // relay-agent directory listing + verified owner lookup for directory and existing channel shared agents
