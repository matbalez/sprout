  ...
  const renderItem = (item: InboxItem, index: number) => {
    ...
    const rowHighlightColor = shouldHighlightBounty
      ? isSelected
        ...
        : "color-mix(in srgb, rgb(16 185 129) 14%, hsl(var(--background)))"
      : isSelected
    ...
    const row = (
      ...
      >
        <button
          ...
          className={cn(
            "absolute inset-0 z-0 block w-full border-l text-left after:pointer-events-none after:absolute after:bottom-0 after:left-[3.625rem] after:right-3 after:h-px after:bg-border/45 after:content-['']",
        ...
        >
          <div className="flex min-w-0 items-start gap-2.5">
            ...
            <div className="min-w-0 flex-1">
              ...
              />
              {item.bounty ? (
                <span className="mt-1 inline-flex shrink-0 items-center rounded-full border border-emerald-500/60 bg-emerald-300 px-1.5 py-0.5 text-2xs font-bold uppercase leading-none text-emerald-950 shadow-sm dark:bg-emerald-400/90">
                  Bounty: {formatBountyAmount(item.bounty.amountSats)}
                </span>
              ) : null}
