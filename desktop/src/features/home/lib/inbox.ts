...
function buildInboxBountyForItem({
  ...
  const bountyMessageId = bountyByEventId.has(item.id)
    ? item.id
    : getReferencedBountyId(item, bountyByEventId);
  const bounty = bountyMessageId ? bountyByEventId.get(bountyMessageId) : null;
  if (!bounty) {
    return null;
  }
