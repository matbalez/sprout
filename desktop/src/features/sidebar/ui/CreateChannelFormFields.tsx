import { ChevronDown, ClockFading, Hash, Wallet } from "lucide-react";
 ...
 * payment controls, and optional template). Rendered inside both the standalone dialog and the
  ...
  return (
    <div className="space-y-5">
      ...
      <div
        ...
        data-testid="create-channel-hive-row"
      >
        <label
          className="min-w-0 cursor-pointer space-y-0.5"
        ...
        >
          ...
          <span className="block text-xs leading-4 text-muted-foreground/65">
            Create a private channel wallet for member payouts.
          </span>
        </label>
        <Switch
          checked={form.hiveChannel}
          data-testid="create-channel-hive"
          disabled={isCreating}
          id="create-channel-hive"
          onCheckedChange={form.setHiveChannel}
        />
      </div>

      <div className="grid gap-3 rounded-xl border border-input bg-muted/40 p-3">
        <div className="flex items-center justify-between gap-3">
          ...
          <Switch
            checked={form.paidJoinEnabled}
            disabled={isCreating || form.hiveChannel}
            id="create-channel-paid-join"
            onCheckedChange={form.setPaidJoinEnabled}
          />
        </div>
        {form.paidJoinEnabled ? (
          <div className="flex items-center gap-2">
            ...
            <Input
              ...
              inputMode="numeric"
              min={1}
              onChange={(event) => form.setPaidJoinAmount(event.target.value)}
              placeholder="100"
              step={1}
              type="number"
              value={form.paidJoinAmount}
            />
          </div>
        ) : null}
        <div className="flex items-center justify-between gap-3">
          ...
          <Switch
            checked={form.paidPostEnabled}
            disabled={isCreating || form.hiveChannel}
            id="create-channel-paid-post"
            onCheckedChange={form.setPaidPostEnabled}
          />
        </div>
        {form.paidPostEnabled ? (
          <div className="flex items-center gap-2">
            <span className="text-sm text-muted-foreground">₿</span>
            <Input
              disabled={isCreating}
              inputMode="numeric"
              min={1}
              onChange={(event) => form.setPaidPostAmount(event.target.value)}
              placeholder="10"
              step={1}
              type="number"
              value={form.paidPostAmount}
            />
          </div>
        ) : null}
      </div>
