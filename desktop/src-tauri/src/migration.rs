...
mod tests {
    ...
    #[test]
    fn reconcile_keeps_empty_mcp_command_for_goose() {
    ...
    #[test]
    fn reconcile_keeps_empty_mcp_command_for_claude() {
        let dir = tempfile::tempdir().unwrap();
        write_agents_json(
            dir.path(),
            &serde_json::json!([{
    ...
    fn reconcile_handles_mixed_records() {
        ...
        let records = read_agents_json(dir.path());
        assert_eq!(
            records[0]["mcp_command"], "",
            "goose should not use a bundled MCP command"
        );
        assert_eq!(
            records[1]["mcp_command"], "",
            "claude should not use a bundled MCP command"
        );
        assert_eq!(
        ...
        assert_eq!(
            records[4]["mcp_command"], "",
            "codex should not use a bundled MCP command"
        );
    }
