...
pub fn spawn_agent_child(
    ...
    let default_agents_to_lexe = crate::wallet::default_agents_to_lexe_payments(app)
        .unwrap_or_else(|error| {
            eprintln!(
                "buzz-desktop: failed to load agent payment settings; defaulting Lexe payments on: {error}"
            );
            true
        });
    ...
    command.env("BUZZ_TOOLSETS", effective_mcp_toolsets);
    if !toolsets_request_payments(effective_mcp_toolsets) {
        command.env_remove("SPROUT_WALLET_BROKER_URL");
        command.env_remove("SPROUT_WALLET_BROKER_TOKEN");
    }
